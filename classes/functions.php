...
function i_own_content($row){
	...
	if(isset($row['postid'])){
		...
		if($myid == $row['userid']){
			...
			//	return true;
			//}
			//return $myid == $one_post['userid'];
			if(isset($one_post['userid']) && $myid == $one_post['userid']){
				return true;
			 }
...
/*			if (is_array($one_post) && isset($one_post['owner'])) {
				...
				// Faire quelque chose si $one_post n'est pas un tableau ou ne contient pas la clé 'owner'
			}
			*/

			if(is_array($row) && isset($row['postid'])){
				...
				if($myid == $row['userid']){
					...
					if(is_array($one_post) && isset($one_post['owner']) && $myid == $one_post['userid']){
						return true;
					}
